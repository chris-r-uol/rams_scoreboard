...
<div class="overlay-root">
  <div class="scorebug">
    ...
    <div class="center-block">
      <div class="game-clock {state.gameClockRunning ? 'running' : ''}">{formatGameClock(state.gameClockSeconds)}</div>
  ...
  {#if state.flagThrown}
    <div class="flag-bar"><span class="flag-icon">⚑</span> FLAG ON THE PLAY</div>
  {/if}
</div>
...
<style>
  ...
  .team-block { position: relative; min-width: 200px; }
  .team-inner { display: flex; align-items: center; gap: 14px; padding: 14px 22px; }
  .team-name { font-size: 15px; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; opacity: 0.95; white-space: nowrap; }
  .team-score { font-size: 40px; font-weight: 900; line-height: 1; font-variant-numeric: tabular-nums; min-width: 2ch; }
  .team-home .team-score { text-align: right; }
  .team-away .team-score { text-align: left; }
  .possession-bar { position: absolute; left: 0; top: 0; width: 4px; height: 100%; opacity: 0.85; }
  .away-bar { left: auto; right: 0; }
  .timeout-dots { display: flex; gap: 4px; align-items: center; }
  .timeout-dot { width: 7px; height: 7px; border-radius: 50%; opacity: 0.15; transition: opacity 0.3s ease; }
  .timeout-dot.active { opacity: 0.85; }
  .center-block {
    background: #0a0a0a; color: #ffffff;
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    padding: 10px 28px; min-width: 150px; gap: 3px;
  }
  .game-clock { font-size: 32px; font-weight: 900; font-variant-numeric: tabular-nums; letter-spacing: -0.02em; line-height: 1; }
  .center-meta { display: flex; align-items: center; gap: 8px; margin-top: 2px; }
  .quarter-tag { font-size: 12px; font-weight: 700; color: #d1d5db; letter-spacing: 0.08em; }
  .meta-sep { width: 1px; height: 10px; background: #4b5563; }
  .play-clock-box {
    font-size: 12px; font-weight: 800; font-variant-numeric: tabular-nums; color: #ffffff;
    background: #dc2626; padding: 2px 8px; border-radius: 5px; line-height: 1.4;
  }
  .play-clock-box.warning { background: #d97706; }
  .play-clock-box.critical { background: #991b1b; animation: pulse 0.6s ease-in-out infinite; }
  .info-strip {
    display: flex; align-items: center; justify-content: center; gap: 10px;
    background: rgba(10,10,10,0.92); padding: 5px 20px;
    border-radius: 0 0 8px 8px; box-shadow: 0 4px 16px rgba(0,0,0,0.4);
  }
  .info-down { font-size: 12px; font-weight: 700; color: #e5e7eb; letter-spacing: 0.04em; }
  .info-sep { width: 1px; height: 12px; background: #4b5563; }
  .info-ballon { font-size: 12px; font-weight: 600; color: #9ca3af; letter-spacing: 0.03em; }
  .flag-bar {
    background: #eab308; color: #000; padding: 5px 18px; border-radius: 6px;
